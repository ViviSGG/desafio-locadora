...
export default function Toyota() {
  ...
  const fetchCars = async () => {
    try {
      ...
      const toyotaCars = response.data.map(car => ({
        ...car,
        image: '/cars/toyota1.jpg' // Substitui com uma imagem fixa de Toyota
      }));

      setInfo(toyotaCars);
      console.log(toyotaCars); // Retorna no console os dados dentro da API
    } catch (error) {
